...
10. **Design system reference for frontend tasks** — any subagent dispatch that includes `frontend-developer` MUST pass `docs/DESIGN.md` and `docs/preview/` as explicit inputs in the dispatch brief. If a screen-specific design file exists in `docs/designs/` for the screen being implemented, include it as well — the agent implements that screen to match the provided design exactly. For screens recorded as DESIGN.md-fallback in the session log, use `docs/DESIGN.md` and `docs/preview/` only. Never dispatch a frontend task without `docs/DESIGN.md` and `docs/preview/` — omitting them causes the agent to invent its own visual language and breaks design consistency across the codebase.
