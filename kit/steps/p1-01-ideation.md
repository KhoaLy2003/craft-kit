...
- **Notes**:
  ...
  - **Q&A before artifact (critical):** Do not write `docs/idea-brief.md` until all clarifying questions are answered and the user has reviewed the draft. The contract is: questions → answers → draft → user review → write. Writing before Q&A completes produces an idea-brief that reflects assumptions, not answers — and requires rework. Fill `templates/01-idea-brief.md`, save to `docs/idea-brief.md`, and mark `Status: approved` only after the user confirms the draft. Do not invoke `writing-plans`.
